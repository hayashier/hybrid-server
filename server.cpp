    ...
    while(1) {
        ...
        for( int i = 0; i < MAX_SESSION; i++) {
            ...
            if(FD_ISSET(accfd[i], &fds)) {
                ...
                std::vector<std::string> header = HTTP1_Response::make_header(1, body_length, is_file_exist, path);
                server_response = HTTP1_Response::make_response(header, message_body);
                if(write(accfd[i], server_response.c_str(), server_response.length()) == -1){
